...
const DASHBOARD_FEATURES: Array<{
  ...
  { title: "فروش سریع (POS)", description: "فروش سریع فروشگاهی با صفحه مخصوص صندوق", icon: "M4 7h16M4 7l1 12h14l1-12M9 11h6m-6 4h4M9 3h6l1 4H8l1-4z", color: "from-emerald-500 to-lime-500", page: "pos" },
  ...
  { title: "اطلاعات توسعه‌دهنده", description: "نام، تماس و ایمیل توسعه‌دهنده نرم‌افزار", icon: "M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z", color: "from-slate-500 to-gray-600", page: "developer" },
];
...
function App() {
  ...
  const [licenseExpiryRefreshing, setLicenseExpiryRefreshing] = useState(false);
  ...
  const loadLicenseRemainingDays = useCallback(async () => {
  ...
  const handleRefreshLicenseExpiry = useCallback(async () => {
    ...
    try {
      ...
      await loadLicenseRemainingDays();
    } catch {
      // ignore; local expiry unchanged
    } finally {
      setLicenseExpiryRefreshing(false);
    }
  }, [user, licenseExpiryRefreshing, loadLicenseRemainingDays]);
  ...
  if (currentPage === "developer") {
    return (
      <DeveloperInfo onBack={() => handleBack("dashboard")} />
    );
  }
